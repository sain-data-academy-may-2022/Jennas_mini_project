...
def food_loop(food_options):
    ...
    if food_options == 0 :
        ...
        database_func.print_specific_db('food', ('product_id, food_name, description, food_price, quantity') , food_headers )
        orders.add_to_order()
...
def orders_loop (order_options):
    ...
    if order_options == 0 :
        ...
        search = menus.get_input("Do you want to search by status or courier?")
        if search == "status":
            database_func.search_by_key('See Delivered or Preparing?', 'status', 'orders', ('order_id', 'status'))
        if search == "courier":
            database_func.print_db('couriers', courier_headers )
